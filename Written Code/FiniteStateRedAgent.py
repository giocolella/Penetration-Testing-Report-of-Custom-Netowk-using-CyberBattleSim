...
class FiniteStateRedAgent(BaseAgent):
    ...
    def _choose_host_and_action(self, action_space: dict, host_options: List[str]):
        ...
        # Gather possible actions & their transition probabilities for this host
        state = self.host_states[chosen_host]['state']
        probs_map = {
            self.action_list[i]: p
            for i, p in enumerate(self.state_transitions_probability[state])
            if p is not None
        }

        invalid_actions = []
        while True:
            # only keep actions that are legal and not previously invalidated
            options = [act for act, ok in action_space['action'].items()
                       if ok and act not in invalid_actions and act in probs_map]
            if options:
                # pull raw probabilities
                raw = [probs_map[act] for act in options]
                total = sum(raw)
                if total > 0:
                    norm = [r/total for r in raw]
                else:
                    # fallback to uniform
                    norm = [1/len(raw)] * len(raw)
                action_class = self.np_random.choice(options, p=norm)
            else:
                # no valid action for this host: try a different host
                remaining = host_options.copy()
                if chosen_host in remaining:
                    remaining.remove(chosen_host)
                return self._choose_host_and_action(action_space, remaining)

            # build parameters for the chosen action
            params = {}
            for name in self.action_params[action_class]:
                opts = [v for v, ok in action_space.get(name, {}).items() if ok]
                if name == 'hostname':
                    hostnm = self.host_states[chosen_host]['hostname']
                    if hostnm is not None:
                        params[name] = hostnm
                    else:
                    ...
                    params[name] = IPv4Address(chosen_host)
                elif name == 'agent':
                    params[name] = self.name
                elif name == 'session' and 'session' in action_space:
                    params[name] = next(iter(action_space['session']))
                elif opts:
            ...
            if params is not None:
                return chosen_host, action_class(**params)
