...
def epsilon_greedy_search(
    ...
    print("\n========== [REPORT FINALE] ==========\n")
    agent = cyberbattle_gym_env._actuator

    for node_id, node in agent.discovered_nodes():
        print(f"\n-- Nodo: {node_id} --")
        
        # OS e ruolo
        os_info = [p for p in node.properties if p.startswith("os:")]
        ruolo = [p for p in node.properties if p.startswith("role:")]
        print(f"  OS: {os_info}")
        print(f"  Ruolo: {ruolo}")

        # Privilegi
        level = agent.get_node_privilegelevel(node_id)
        print(f"  Privilegio: {level.name}")

        # Servizi
        services = [s.name for s in node.services]
        print(f"  Servizi aperti: {services}")

        # Vulnerabilità
        print(f"  Vulnerabilità presenti: {list(node.vulnerabilities.keys())}")

        # Owned string se presente
        if hasattr(node, 'owned_string') and node.owned_string:
            print(f"  Owned string: {node.owned_string}")
